...
public class DaoEmployee {
    ...
    public void actualizar(Employees empleadoActualizado){
        ...
        String sql = "update employees set first_name= ?, last_name= ?, email= ?, password= ?, phone_number= ?,hire_date= ?,job_id= ?,salary= ? where employee_id = ?";

        try(Connection connection = DriverManager.getConnection(url,username,password);
            PreparedStatement pstmt = connection.prepareStatement(sql)){
            String fullName= empleadoActualizado.getFullNameEmployee().trim();
            String[] words = fullName.split(" ");
            nombre = words[0];
            apellido = words[1];
            pstmt.setString(1,nombre);
            pstmt.setString(2,apellido);
            pstmt.setString(3,empleadoActualizado.getCorreo());
            pstmt.setString(4,empleadoActualizado.getPassword());
            pstmt.setString(5,empleadoActualizado.getNumeroDeTelefono());
            pstmt.setString(6,empleadoActualizado.getHire_date());
            pstmt.setString(7,empleadoActualizado.getJobId());
            pstmt.setInt(8,empleadoActualizado.getSalario());
            pstmt.setInt(9,empleadoActualizado.getEmployeeId());
